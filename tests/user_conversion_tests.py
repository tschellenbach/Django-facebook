...
class UserConversionTest(unittest.TestCase):
    def test_settings(self):
        ...
        required_settings = ['FACEBOOK_APP_SECRET', 'FACEBOOK_APP_ID']
        for s in required_settings:
            if not getattr(settings, s, False):
                raise ValueError('Settings %s is required and is not found in django.conf.settings' % s)
